...
export function isLMStudioURL(baseURL?: string): boolean {
  ...
  return (
    ...
    u.includes("lmstudio") ||
    /localhost:1234|127\.0\.0\.1:1234/.test(u)
  );
}
...
function parseLMStudioRecord(raw: LMStudioRaw, requestedId?: string): ModelRuntimeInfo {
  ...
  const paramBillions =
    parseParamBillions(raw.params_string) ??
    parseParamBillionsFromModelId(modelId);
  const isLoaded =
    raw.state === "loaded" ||
    Boolean(raw.loaded_instances && raw.loaded_instances.length > 0);
  const quant =
...
export async function fetchLMStudioModelRuntime(
  ...
  const match = models.find(
    (m) =>
      modelIdsMatch(m.id || m.key || m.path || "", modelId)
  );
...
export function isSmallModelFromConfig(
  cfg: Pick<
    Config,
    "model" | "maxTokens" | "smallModelMode" | "modelParamBillions"
  >
): boolean {
...
export async function enrichConfigWithRuntime(cfg: Config): Promise<Config> {
  ...
  if (isLMStudioURL(cfg.baseURL)) {
    ...
    const paramSmall =
      runtime.paramBillions !== undefined && runtime.paramBillions <= 8;
    const smallModelMode =
      cfg.smallModelMode ?? (paramSmall || isSmallModel(cfg.model));
