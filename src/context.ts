...
export function detectContext(workspace: string): RepoContext {
  const ctx: RepoContext = { isGit: false, files: [], projectType: "unknown", primaryLanguage: "unknown" };
  try {
    execSync("git rev-parse --git-dir", { cwd: workspace, stdio: "ignore" });
    ctx.isGit = true;
    ctx.branch = execSync("git branch --show-current", { cwd: workspace, encoding: "utf-8", stdio: "pipe" }).trim();
  } catch {
    /* not git */
  }
