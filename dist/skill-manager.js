...
export class SkillManager {
     ...
     */
    unload(name, messages, smallModel, onUpdate) {
     ...
     */
    syncSkillMessages(messages, smallModel) {
     ...
     */
    autoLoad(userText, messages, smallModel, onUpdate) {
        const allSkills = loadSkills();
        const matched = matchSkillTriggers(userText, allSkills);
        const newlyLoaded = [];
        for (const skill of matched) {
            if (this._autoLoadedSkills.has(skill.name))
                continue;
            if (this.activeSkills.has(skill.name))
                continue;
