 ...
 */
export function applySubAgentDefaults(cfg) {
 ...
 */
export function loadConfig(pathOrConfig) {
    ...
    if (pathOrConfig && typeof pathOrConfig === 'object') {
        const filteredConfig = Object.fromEntries(Object.entries(pathOrConfig).filter(([_, v]) => v !== undefined));
    ...
    if (process.env.QWEN_BASE_URL)
        cfg.baseURL = sanitizeBaseURL(process.env.QWEN_BASE_URL);
    ...
    if (process.env.QWEN_WORKSPACE)
        cfg.workspace = process.env.QWEN_WORKSPACE;
    ...
    if (process.env.QWEN_THEME)
        cfg.theme = process.env.QWEN_THEME;
    ...
    if (process.env.QWEN_TOOL_CACHE_ENABLED === '0' ||
    ...
    if (smallModel) {
        ...
        if (detected) {
            console.warn(`Config warning: smallModelMode is set to false, but "${cfg.model}" appears to be a small model (≤8B). ` +
...
export function validateConfig(cfg) {
    ...
    if (!isLocal &&
        ...
        warnings.push('apiKey looks like a malformed OpenAI-style key — expected ~51 chars, got ' +
    ...
    if (cfg.contextCompactThreshold !== undefined) {
        if (cfg.contextCompactThreshold < 0 || cfg.contextCompactThreshold > 1) {
            errors.push(`contextCompactThreshold must be between 0 and 1, got ${cfg.contextCompactThreshold}`);
        }
    }
    if (cfg.contextSummaryReservedPercent !== undefined) {
        if (cfg.contextSummaryReservedPercent < 0 || cfg.contextSummaryReservedPercent > 1) {
            errors.push(`contextSummaryReservedPercent must be between 0 and 1, got ${cfg.contextSummaryReservedPercent}`);
        }
    }
    ...
    if (cfg.contextMaxHistoryTokens !== undefined) {
        if (cfg.contextMaxHistoryTokens < 100 || cfg.contextMaxHistoryTokens > 1000000) {
            errors.push(`contextMaxHistoryTokens must be between 100 and 1000000, got ${cfg.contextMaxHistoryTokens}`);
        }
    }
    // Security configuration validation
    if (cfg.securityMaxFileSize !== undefined) {
        if (cfg.securityMaxFileSize < 1 || cfg.securityMaxFileSize > 100 * 1024 * 1024) {
            errors.push(`securityMaxFileSize must be between 1 and 104857600 (100MB), got ${cfg.securityMaxFileSize}`);
        }
    }
    if (cfg.securityMaxBatchFiles !== undefined) {
        if (cfg.securityMaxBatchFiles < 1 || cfg.securityMaxBatchFiles > 1000) {
            errors.push(`securityMaxBatchFiles must be between 1 and 1000, got ${cfg.securityMaxBatchFiles}`);
        }
    }
 ...
 */
export function saveApiKeyToEnv(envVarName, apiKey, envPath) {
    try {
 ...
 */
export function getApiKey(envVarName) {
 ...
 */
export function removeApiKeyFromEnv(envVarName) {
    try {
 ...
 */
export function toggleSkillInConfig(name) {
    try {
 ...
 */
export function getSkillEnabledFromConfig(name) {
    const config = loadSkillConfig();
 ...
 */
export function getAllConfiguredSkills() {
    const config = loadSkillConfig();
    const map = new Map();
 ...
 */
export function saveConfigFile(updates, scope = 'global', workspace) {
