...
export async function cmdRun(argv) {
    ...
    if (!prompt?.trim()) {
        cliError('No prompt provided.', '  nanogent run --prompt "your task here"\n' +
            '  echo "your task" | nanogent run --stdin --workspace .');
    }
    const cfg = loadConfig();
    if (values.workspace)
        cfg.workspace = resolve(values.workspace);
    if (values.model)
        cfg.model = values.model;
    if (values['base-url'])
        cfg.baseURL = values['base-url'];
    ...
    if (values.json) {
        ...
        if (values.verbose && result.usage.input_tokens + result.usage.output_tokens > 0) {
            console.error(`tokens: ${result.usage.input_tokens} in / ${result.usage.output_tokens} out · rounds: ${result.rounds}`);
        }
    }
    return ok ? 0 : 1;
}
