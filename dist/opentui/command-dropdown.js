...
export function CommandDropdown({ inputValue, theme, onPick, onSubmit, skillCommands: propSkillCommands, }) {
    ...
    const getActualIndex = useCallback((displayIndex) => {
        let count = 0;
        for (const item of displayItems) {
            if (item.type === 'command') {
                if (count === displayIndex) {
                    return filtered.findIndex((c) => c.name === item.name && c.description === item.description);
                }
                count++;
            }
        }
    ...
    if (!open)
        return null;
    if (commandCount === 0) {
        return (_jsx("box", { borderStyle: "single", borderColor: theme.borderColor, paddingX: 1, height: 3, flexShrink: 0, backgroundColor: theme.bgPanel, children: _jsxs("text", { fg: theme.mutedFg, children: ["(no match \u2014 Enter sends \"", inputValue, "\")"] }) }));
    }
    const pad = Math.max(...filteredBuiltin.map((c) => c.name.length), ...filteredSkills.map((c) => c.name.length));
    const headerCount = (filteredBuiltin.length > 0 ? 1 : 0) +
        (filteredSkills.length > 0 && filteredBuiltin.length > 0 ? 1 : 0);
    const visibleRows = Math.min(commandCount + headerCount, 6);
    return (_jsx("scrollbox", { ref: scrollRef, flexDirection: "column", borderStyle: "single", borderColor: theme.borderColor, paddingX: 1, height: Math.min(visibleRows + 2, 8), flexShrink: 0, backgroundColor: theme.bgPanel, children: displayItems.map((item, _displayIndex) => {
            if (item.type === 'header') {
                return (_jsx("text", { fg: theme.mutedFg, marginTop: 1, children: `  ${item.name}` }, `header-${item.name}`));
            }
            if (item.type === 'command') {
                ...
                return (_jsx("text", { id: itemId(actualIndex), fg: isSel ? theme.headerFg : item.isSkill ? theme.agentFg : theme.inputFg, bg: isSel ? theme.bgSelected : undefined, children: `${isSel ? '▸ ' : '  '}${padded}   ${item.description}` }, item.name));
            }
            return null;
        }) }));
}
