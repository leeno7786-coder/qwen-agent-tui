...
export async function runTui() {
    ...
    try {
        ...
        renderer.root.add(new TextRenderable(renderer, {
            content: 'Error: Failed to load configuration.\n' +
    ...
    if (!hasKey && !isLocal) {
        ...
        const { TextRenderable } = await import('@opentui/core');
        renderer.root.add(new TextRenderable(renderer, {
            content: 'Error: No API key configured for remote provider.\n' +
    ...
    createRoot(appRenderer).render(_jsx(App, { renderer: appRenderer }));
}
