...
export function isLMStudioURL(baseURL) {
    if (!baseURL)
        return false;
    const u = baseURL.toLowerCase();
    return (u.includes('lm-studio') || u.includes('lmstudio') || /localhost:1234|127\.0\.0\.1:1234/.test(u));
}
/** Base URL for LM Studio REST v0 (strip trailing /v1). */
export function lmStudioRestBase(baseURL) {
    return baseURL.replace(/\/+$/, '').replace(/\/v1\/?$/i, '');
}
/** Parse "7B", "270M", "0.5B" → billions of parameters. */
export function parseParamBillions(paramsString) {
    if (!paramsString)
        return undefined;
...
export function parseParamBillionsFromModelId(modelId) {
    const lower = modelId.toLowerCase();
    const m = lower.match(/\b(0\.5|1\.5|1|2|3|4|7|8|9|13|14|27|32|70)[-.]?b\b/);
    if (m)
        return parseFloat(m[1]);
    if (lower.includes('nano') || lower.includes('4b'))
        return 4;
    return undefined;
}
...
function parseLMStudioRecord(raw, requestedId) {
    ...
    const paramBillions = parseParamBillions(raw.params_string) ?? parseParamBillionsFromModelId(modelId);
    const isLoaded = raw.state === 'loaded' || Boolean(raw.loaded_instances && raw.loaded_instances.length > 0);
...
function normalizeLMStudioList(body) {
 ...
 */
export async function fetchLMStudioModelRuntime(baseURL, modelId) {
    ...
    if (single && typeof single === 'object' && !Array.isArray(single)) {
        ...
        if (list.length === 1)
            return parseLMStudioRecord(list[0], modelId);
        if ('id' in single || 'key' in single) {
            return parseLMStudioRecord(single, modelId);
        }
    }
    ...
    if (match)
        return parseLMStudioRecord(match, modelId);
    return null;
}
/**
...
export async function fetchLMStudioModels(baseURL) {
    ...
    return models
        ...
        const params = info.paramBillions !== undefined
...
function formatContextLabel(info) {
    ...
    if (info.maxContextLength && info.contextLength && info.maxContextLength !== info.contextLength) {
        const maxK = info.maxContextLength >= 1000
 ...
 */
export function pickSubAgentModel(models, mainModelId) {
    ...
    const rank = (m) => {
        ...
        if (/0\.8\s*b|0\.8b|800m|qwen3\.5[-:]?0\.8/i.test(id))
            return 0;
        if (m.paramBillions !== undefined && m.paramBillions <= 1.5) {
            return 10 + m.paramBillions;
        }
        if (/\b(1|1\.5)[-.]?b\b/.test(id))
            return 20;
        return 100;
    };
    return [...pool].sort((a, b) => rank(a) - rank(b))[0];
}
export function isSmallModelFromConfig(cfg) {
    if (cfg.smallModelMode === true)
        return true;
    if (cfg.smallModelMode === false)
        return false;
 ...
 */
export async function enrichConfigWithRuntime(cfg) {
    if (!isLocalProvider(cfg.baseURL) || !cfg.model)
        return cfg;
...
export function runtimeContextFromConfig(cfg) {
    return {
