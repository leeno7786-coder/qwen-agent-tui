...
export class SecurityManager {
     ...
     */
    validateFileAccess(path, operation = 'read') {
        ...
        if (this.config.allowedPaths.length > 0) {
            isExplicitlyAllowed = this.config.allowedPaths.some((pattern) => this.pathMatchesPattern(relPath, pattern));
 ...
 */
export function createSecurityManager(config, workspace) {
