...
export class MemoryGraph {
        ...
        try {
            for (const item of readdirSync(directory)) {
                ...
                if (stat.isDirectory()) {
                    ...
                    if (this.options.excludedPaths?.some((p) => item === p || normPath.includes(`/${p}/`) || normPath.endsWith(`/${p}`))) {
                        continue;
                    }
                    if (!this.options.includedPaths) {
                        ...
                        if (this.options.includedPaths.some((p) => item === p || normPath.includes(`/${p}/`) || normPath.endsWith(`/${p}`))) {
                            files.push(...this.findFiles(fullPath));
                        }
                    }
                }
     ...
     */
    processTypeScriptFile(filePath, content, fileNode) {
    ...
    processTSStatement(node, fileNode, sf) {
    ...
    processTSImport(node, fileNode, sf) {
    ...
    addImportNode(name, modulePath, fileNode, node, sf) {
        const importNode = {
    ...
    processTSExport(node, fileNode, sf) {
    ...
    processTSExportDefault(node, fileNode, sf) {
        const fn = {
    ...
    processTSClass(node, fileNode, sf) {
    ...
    addHeritageRef(id, type, name, fileNode, node, sf) {
    ...
    processTSFunction(node, fileNode, sf) {
    ...
    processTSVariables(node, fileNode, sf) {
        for (const decl of node.declarationList.declarations) {
            ...
            if (decl.initializer && ts.isFunctionLike(decl.initializer)) {
                this.extractCalls(decl.initializer, n, fileNode, sf);
            }
        }
    }
    processTSType(node, fileNode, sf, subType) {
    ...
    processTSEnum(node, fileNode, sf) {
        const en = {
    ...
    processTSModule(node, fileNode, sf) {
        const mn = {
    ...
    extractCalls(container, parentNode, fileNode, sf) {
     ...
     * Process JSON file
     */
    processJsonFile(filePath, content, fileNode) {
        try {
     ...
     */
    processMarkdownFile(filePath, content, fileNode) {
     ...
     * Process YAML file
     */
    processYamlFile(filePath, content, fileNode) {
    ...
    queryPaths(criteria) {
        if (typeof criteria === 'object') {
            ...
            const maxDepth = criteria.maxDepth || 5;
            if (from && to) {
     ...
     */
    findPaths(fromId, toId, maxDepth) {
        const paths = [];
        const queue = [
     ...
     */
    querySemantic(criteria) {
        const results = [];
        if (typeof criteria === 'string')
            return results;
        const relatedTo = criteria.relatedTo;
        if (relatedTo) {
    ...
    getCommunities() {
        ...
        for (const [commId, nodeIds] of commNodes) {
            ...
            for (const [, edge] of this.edges) {
                ...
                if (sComm === commId && tComm === commId)
                    internalEdges++;
                else if (sComm === commId || tComm === commId)
                    externalEdges++;
            }
            const community = {
                id: commId,
    ...
    getGodNodes(n = 10) {
        ...
        for (const [, edge] of this.edges) {
            if (!degreeMap.has(edge.source))
                degreeMap.set(edge.source, { in: 0, out: 0, total: 0 });
            if (!degreeMap.has(edge.target))
                degreeMap.set(edge.target, { in: 0, out: 0, total: 0 });
     ...
     */
    getSurprisingConnections(limit = 20) {
        this.detectCommunities();
        const results = [];
        for (const [, edge] of this.edges) {
